...
(function (gameLogic) {
    ...
    function getWinner(board) {
        ...
        if (isWinner(wrow, wcol, board))
            return 'W';
        if (isWinner(rrow, rcol, board))
            return 'R';
        return '';
    }
    ...
    gameLogic.getStartRC = getStartRC;
    ...
    function createMove(board, brow, bcol, arow, acol, turnIndexBeforeMove) {
        ...
        if (winner !== '') {
            // Game over.
            firstOperation = { endMatch: { endMatchScores: (winner === 'W' ? [1, 0] : [0, 1]) } };
        }
        else {
    ...
    gameLogic.getCheckNum = getCheckNum;
    function isMoveLegal(board, brow, bcol, arow, acol, turnIndexBeforeMove) {
        ...
        if (board[brow][bcol] === enemy) {
            return false;
        }
        if (Math.abs(brow - arow) !== Math.abs(bcol - acol) && brow !== arow && bcol !== acol) {
            return false;
        }
        var distance;
        var startR;
    ...
    gameLogic.isMoveLegal = isMoveLegal;
    ...
    function getRiddles() {
        return [getExampleMoves(1, { board: [['W', '', '', '', 'R', '', '', ''],
                    ['W', 'W', 'W', '', '', '', '', ''],
                    ['W', 'R', 'W', 'R', '', '', '', ''],
                    ['W', '', 'R', '', '', '', '', 'R'],
                    ['W', '', '', 'R', '', 'W', '', ''],
